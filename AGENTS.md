...
- **Environment variables**: `NEXT_PUBLIC_DOCS_URL` (docs link target), `SESSION_SECRET` (demo session signing), `NEXT_PUBLIC_API_URL` (optional API base URL for `@repo/api-client`). See `.env.example`.
